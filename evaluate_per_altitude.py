...
ALTITUDES = ["60","70","80","90","100","110","120","130"]
...
for img_path in tqdm(image_files):
    ...
    altitude = parse_altitude(img_path)
    ...
    for obj in gt_objects:
        results[altitude]["gt_count"][obj["class"]] += 1
    ...
    scale_tensor = torch.tensor([W, H, W, H]).to(DEVICE)
    ...
    for box, logit, phrase in zip(boxes, logits, phrases):
        ...
        results[altitude]["predictions"][cls].append({
...
for alt in ALTITUDES:
    ...
    for cls in CLASSES:
        ...
        predictions = sorted(
            results[alt]["predictions"][cls],
            key=lambda x: x["score"],
            reverse=True
        )

        total_gt = results[alt]["gt_count"][cls]
    ...
    if len(aps) > 0:
        print("-"*50)
        print(f"mAP@{alt}m: {np.mean(aps):.4f}")